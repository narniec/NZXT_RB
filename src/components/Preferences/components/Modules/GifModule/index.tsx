...
const tenorApiKey = import.meta.env.VITE_TENOR_API as string
const giphyApiKey = import.meta.env.VITE_GIPHY_API as string
const gf = new GiphyFetch(giphyApiKey ?? 'demo')
...
export const GifModule = () => {
  ...
  const fetchGifs = (offset: number) =>
    gf.search((debouncedTerm as string) || 'trending', { offset, limit: 10, type: 'gifs' })
    ...
    <Container className={`${preferencesStore.current.gif.url ? '--is-gifSet' : ''}`}>
      <div className="preview">
        ...
        <Range
          ...
          onChange={value => preferencesStore.updateGif({ size: value })}
        />

        <Range
          label="Blur"
          value={preferencesStore.current.gif.blur}
          onChange={value => preferencesStore.updateGif({ blur: value })}
        />

        <Range
          label="Brightness"
          value={preferencesStore.current.gif.brightness}
          onChange={value => preferencesStore.updateGif({ brightness: value })}
        />

        <Range
          label="Contrast"
          value={preferencesStore.current.gif.contrast}
          onChange={value => preferencesStore.updateGif({ contrast: value })}
        />

        <Range
          label="Alpha"
          value={preferencesStore.current.gif.alpha}
          onChange={value => preferencesStore.updateGif({ alpha: value })}
        />
      </div>
